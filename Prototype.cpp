...
//  
//
//  Created by John Public on 2/29/16.
//
//
...
#define NEIGHBORLIST_OPTION 2
#define VERLET_OPTION 3
#define ALL_OPTION 4
...
double calculate_energy_brute_force(double **coords, const int n_atoms, const double *box_size, const double sigma, const double epsilon)
{
...
//---------------------------------------------------------------------------------------------
void calculate_Chris_Woods(double** coords, double** old_coords)
{
    ...
    for (int move=1; move<=num_moves; move = move + 1)
    {
        ...
        const double old_energy = calculate_energy_brute_force(coords, n_atoms, box_size, sigma, epsilon);
        ...
        const double new_energy = calculate_energy_brute_force(coords, n_atoms, box_size, sigma, epsilon);
...
/*
    @param argv: ./metropolis <Algorithm Option> <# of atoms> <# of moves>-----------------------
*/
int main(int argc, const char** argv)
{
    if(argc != 4)
    {
        cout << "\nUsage: ./metropolis <Algorithm Option> <# of atoms> <# of moves>";
        exit(0);
    }
    
    option = atoi(argv[1]);
    n_atoms = atoi(argv[2]);
    num_moves = atoi(argv[3]);
    
    //Set up the coordinates randomly for each atom
    ...
    if(option == NORMAL_OPTION)
    {
        // Print the initial PDB file
        print_pdb(coords, n_atoms, 0);
        calculate_Chris_Woods(coords, old_coords);
        return 0;
        
    }
    
    if(option == NEIGHBORLIST_OPTION)
    {
        // Print the initial PDB file
        print_pdb(coords, n_atoms, 0);
        return 0;
